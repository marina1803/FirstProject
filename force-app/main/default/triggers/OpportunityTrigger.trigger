  ...
  if (Trigger.isAfter && Trigger.isUpdate) {
   ...
   //Requirement:
    ...
    if(Trigger.isBefore && Trigger.isDelete){
        ...
        //using Trigger.old bcz Trigger.new is not avaialable in the read event
        for(Opportunity each : Trigger.old) {
 
        // If the opportunity is in Closed Won stage 
            if(each.StageName=='Closed Won'){
                each.addError('You can not delete Op is Closed Wion Stage ');
            }
        }
    ...
    //Requirement:
    //If the opportunity Ammount is less that 0
    //It should not be entered
    //Throw error message : You can not enter negative ammount
